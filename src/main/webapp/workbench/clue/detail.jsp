...
<script type="text/javascript">
	...
	$(function(){
		...
		// 复选控制全选
		$("#activityRelationBody").on("click", $("input[name=xz]"), function (){
		...
		// 为关联市场活动的模态窗口的查询绑定事件（回车）
		$("#activityName").keydown(function (event){
			if (event.keyCode==13){
				$.ajax({
					url : "${pageContext.request.contextPath}/workbench/clue/searchActivityByNameNoRelation.do",
					data : {
	...
	function unbind(id){
		//alert(id);
		$.ajax({
			url : "${pageContext.request.contextPath}/workbench/clue/unbind.do",
			data : {
				"id" : id
			},
			type : "post",
	...
	// 关联市场活动窗口
	function showActivityNoRelation(){
		$.ajax({
			url : "${pageContext.request.contextPath}/workbench/clue/getActivityNoRelation.do",
			data : {
				"clueId" : "${requestScope.clue.id}"
			},
			type : "get",
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='${pageContext.request.contextPath}/workbench/clue/convert.jsp?id=${clue.id}&fullname=${clue.fullname}&appellation=${clue.appellation}&company=${clue.company}&owner=${clue.owner}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a href="javascript:void(0);" onclick="showActivityNoRelation()" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
