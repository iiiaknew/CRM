    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#openSearchModalBtn").click(function () {
                $.ajax({
                    url: "${pageContext.request.contextPath}/workbench/clue/getActivityRelation.do",
                    data: {
                        "clueId": "${param.id}"
                    },
                    type: "get",
            ...
            $("#activityName").keydown(function (event) {
                if (event.keyCode == 13) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath}/workbench/clue/getActivityRelationByActivityName.do",
                        data: {
                            "clueId": "${param.id}",
            ...
            $("#convertBtn").click(function (){
                ...
                if ($("#isCreateTransaction").prop("checked")){
                    ...
                    //alert("不需要创建交易");
                    window.location.href="${pageContext.request.contextPath}/workbench/clue/convert.do?clueId=${param.id}";
                }
            })

            // 为提交市场活动按钮绑定事件 填写两项信息（id，name）
            $("#submitActivityBtn").click(function (){
                var $xz = $("input[name=xz]:checked");
                var id = $.trim($xz.val());
                var name = $.trim($("#a"+id).html());
...
<div id="create-transaction2"
    ...
    <form action="${pageContext.request.contextPath}/workbench/clue/convert.do" id="tranForm" method="post">
        <%-- 创建一个标记给后台 让后台判断是否接收表单中的数据 --%>
